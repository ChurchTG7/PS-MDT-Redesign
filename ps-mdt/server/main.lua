...
--------------------------------
-- Mugshot, Clockin, and Incident webhooks are now configured in shared/config.lua
local FivemerrMugShot = Config.FivemerrMugShot
local FivemerrApiKey = Config.FivemerrApiKey
...
QBCore.Functions.CreateCallback('ps-mdt:server:MugShotWebhook', function(source, cb)
    local mode = Config.MugShotWebhookMode
    
    if mode == 'fivemerr' then
        if FivemerrMugShot == '' or FivemerrApiKey == 'YOUR API KEY HERE' then
            print("\27[33m[WARNING] Fivemerr is not configured. Set Config.FivemerrMugShot and Config.FivemerrApiKey in shared/config.lua\27[0m")
            cb('', '')
        else
            cb(FivemerrMugShot, FivemerrApiKey)
        end
    elseif mode == 'discord' or mode == 'hybrid' then
        if MugShotWebhook == '' then
            print("\27[33m[WARNING] MugShot webhook is empty but mode is set to '" .. mode .. "'. Set Config.MugShotWebhook in shared/config.lua\27[0m")
            cb('', '')
        else
            cb(MugShotWebhook, '')
        end
    elseif mode == 'database' then
        -- Database only mode - no external image storage
        cb('database', '')
    else
        print("\27[31m[ERROR] Invalid MugShotWebhookMode: '" .. tostring(mode) .. "'. Use 'fivemerr', 'discord', 'database', or 'hybrid'\27[0m")
        cb('', '')
    end
end)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    -- Check Mugshot configuration
    local mugMode = Config.MugShotWebhookMode
    if mugMode == 'fivemerr' and (Config.FivemerrMugShot == '' or Config.FivemerrApiKey == 'YOUR API KEY HERE') then
        print("\27[33m[WARNING] Fivemerr is not configured. Set Config.FivemerrMugShot and Config.FivemerrApiKey in shared/config.lua\27[0m")
    elseif (mugMode == 'discord' or mugMode == 'hybrid') and Config.MugShotWebhook == '' then
        print("\27[33m[WARNING] MugShot webhook is empty but mode is set to '" .. mugMode .. "'. Set Config.MugShotWebhook in shared/config.lua\27[0m")
    end
    
    -- Check Clockin configuration - auto-switch to database mode if webhook is not configured
    if (Config.ClockinWebhookMode == 'discord' or Config.ClockinWebhookMode == 'hybrid') and Config.ClockinWebhook == '' then
        print("\27[33m[WARNING] Clockin webhook is empty but webhook mode is set to '" .. Config.ClockinWebhookMode .. "'. Switching to database mode.\27[0m")
        Config.ClockinWebhookMode = 'database'
    end
...
AddEventHandler('playerDropped', function(reason)
    ...
    -- Auto clock out if the player is off duty
     if IsPoliceOrEms(job) and PlayerData.job.onduty then
		-- Only update database if mode is 'database' or 'hybrid'
		if Config.ClockinWebhookMode == 'database' or Config.ClockinWebhookMode == 'hybrid' then
		...
		-- Only send to Discord if mode is 'discord' or 'hybrid'
		if Config.ClockinWebhookMode == 'discord' or Config.ClockinWebhookMode == 'hybrid' then
...
QBCore.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    ...
    if Config.ClockinWebhookMode == 'discord' then
        TriggerClientEvent('QBCore:Notify', source, "Leaderboard requires database mode. Change ClockinWebhookMode to 'database' or 'hybrid' in config.", 'error')
        return
    end
    ...
    -- Only send to Discord if webhook mode allows it
    if Config.ClockinWebhookMode == 'discord' or Config.ClockinWebhookMode == 'hybrid' then
        ...
        -- Just print to console if no webhook
        print("MDT Leaderboard requested but Discord webhook is disabled.")
        TriggerClientEvent('QBCore:Notify', source, "MDT leaderboard generated (check console).", 'success')
    end
end)
...
RegisterNetEvent("ps-mdt:server:ClockSystem", function()
    ...
    if PlayerData.job.onduty then
        ...
        -- Only insert to database if mode is 'database' or 'hybrid'
        if Config.ClockinWebhookMode == 'database' or Config.ClockinWebhookMode == 'hybrid' then
